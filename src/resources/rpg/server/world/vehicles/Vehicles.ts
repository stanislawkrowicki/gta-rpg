...
import VehicleSchema, {
    VehicleOwnerType,
} from '../../../../../db/MainDB/schemas/vehicles/Vehicle.schema'
        ...
        const entranceAcknowledgeStateNotifier = alt.setInterval(() => {
            for (let i = 0; i < World.players.length; ++i) {
                ...
                for (let j = 0; j < World.vehicles.length; ++j) {
                    ...
                    if (
                        Vector3.getDistanceBetweenTwoVectors(
                            player.wrapped.pos,
                            vehicle.wrapped.pos
                        ) < 10
                    ) {
    ...
    static spawnWorldVehicleFromDB(id: string, pos: alt.Vector3, rot: alt.Vector3) {
        ...
        MainDB.collections.vehicles
            ...
            .then((vehicle) => {
                ...
                QuickDB.client
                    .execute(['SET', `${Vehicles.REDIS_VEHICLE_KEY}:${vehicle.id}`, 0])
                    .catch((err) =>
            ...
            .catch((err) =>
                Logger.logCaughtError('vehicles', err, 'spawnWorldVehicleFromDB find caught')
            )
    }
    ...
    static despawnWorldVehicle(wrapper: Vehicle) {
        MainDB.collections.vehicles
            ...
            .then(() => {
                ...
                QuickDB.client
                    .execute(['DEL', `${Vehicles.REDIS_VEHICLE_KEY}:${wrapper.id}`])
                    .catch((err) =>
                        Logger.logCaughtError(
                            'vehicles',
                            err,
                            'Failed to delete vehicle from Redis'
                        )
                    )
            })
            .catch((err) =>
                Logger.logCaughtError('vehicles', err, 'Error while despawning world vehicle')
            )
    }

    static async spawnVehiclesInWorld() {
        const vehiclesInWorld = (await QuickDB.client.execute([
            'KEYS',
            `${Vehicles.REDIS_VEHICLE_KEY}:*`,
        ])) as string[]
        ...
        for (let batch = 0; batch < Math.ceil(vehiclesInWorld.length / BATCH_SIZE); batch++) {
            ...
            MainDB.collections.vehicles
                ...
                .then((vehiclesFromDB) => {
                    VEHICLES_FROM_DB_LOOP: for (let i = 0; i < vehiclesFromDB.length; i++) {
                        const vehicle = vehiclesFromDB[i]

                        if (vehicle.position === undefined) {
                            QuickDB.client
                                .execute(['DEL', `${Vehicles.REDIS_VEHICLE_KEY}:${vehicle.id}`])
                                .catch((err) =>
                                    Logger.logCaughtError(
                                        'vehicles',
                                        err,
                        ...
                        for (
                            let alreadySpawnedIndex = 0;
                            alreadySpawnedIndex < vehiclesAlreadySpawned.length;
                            alreadySpawnedIndex++
                        ) {
                            ...
                            if (
                                (
                                    vehiclesAlreadySpawned[alreadySpawnedIndex].getMeta(
                                        'wrapper'
                                    ) as Vehicle
                                ).id === vehicle.id
                            )
                ...
                .catch((err) => {
                    Logger.logCaughtError(
                        'vehicles',
                        err,
                        'Error while finding and spawning vehicles'
                    )
                })
        }
    }
